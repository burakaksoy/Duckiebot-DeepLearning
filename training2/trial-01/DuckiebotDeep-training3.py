...
if __name__ == "__main__":
    ...
    num_of_img = 5378 # Total data images
    num_of_test_img = 428 # Validation data amount
    num_of_training_img = num_of_img - num_of_test_img # 4950
    
    n_rate = 0.0001 # Initial Learning rate hyper parameter
    batch_size = 275 # Training batch size hyper parameter
    ...
    with open(filename) as openfileobject:
        ...
        for line in openfileobject:
            line = line.rstrip() # Remove new line char '/n' from the end
            line_elements = line.split(",") # Create a list seperated from commas
            line_elements = [float(i) for i in line_elements] # Conver strings to float numbers in list
            labels[i,:] = np.array(line_elements[-2:])*100 # Last two elements are vel_left and vel_right, multiply by 100 for easier loss 
            i += 1
    ...
    initializer = tf.contrib.layers.xavier_initializer(dtype=tf.dtypes.float32)    
...
#    M = tf.cast(M, tf.float32)
    ...
    err_avr = tf.reduce_mean(err_mat,axis=0) # vel_right and vel_right averaga errors separetly
    # Define Loss
...
#            num_batches = 2 # TODO: DELETE IT AFTER, DEBUG 
            for i in range(num_batches):
                ...
                if (i % 9 == 0) or ((i_e+1 == max_epochs) and (i+1 == num_batches)):
